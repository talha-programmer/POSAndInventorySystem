...
class Inventory_summary extends Report
{
	...
	{
		$this->db->select($this->Item->get_item_name('name') . ', items.item_number, items.category, item_quantities.quantity,  items.reorder_level, stock_locations.location_name, items.cost_price, items.unit_price, (items.cost_price * item_quantities.quantity) AS sub_total_value');
